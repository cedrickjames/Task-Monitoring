                ...
                <div class="card_body">
                    ...
                    <div class="overflow-x">
                        ...
                        <table style="width:100%;" id="filtertable" class="table datacust-datatable Table">
                            <thead  class="thead-dark">
                                <tr>
                                    ...
                                    <th style="width:5%;" >W1</th>
                                    <th style="width:5%;" >W2</th>
                                    <th style="width:5%;" >W3</th>
                                    <th style="width:5%;" >W4</th>
                                    <th style="width:5%;" >W5</th>
                      ...
                      // echo("<script>console.log('ok');</script>");
                                 ...
                                 ?></td>
                                <td><?php $taskID = $data['usertaskID']; $month = date("F");  $selectUserTask = "SELECT * FROM finishedtask WHERE taskID = '$taskID' AND `month` = '$month' AND `year` = '$year';";$result = mysqli_query($con, $selectUserTask);$weekNumber = '';while($userRow = mysqli_fetch_assoc($result)){ $weekNumber = $userRow['week']; }if ($weekNumber == "week 2" ){$weeknumber = $weekNumber; echo '<span class="mode mode_on">done</span>';;}?></td>
                                <td><?php $taskID = $data['usertaskID']; $month = date("F"); $selectUserTask = "SELECT * FROM finishedtask WHERE taskID = '$taskID' AND `month` = '$month' AND `year` = '$year';";$result = mysqli_query($con, $selectUserTask);$weekNumber = '';while($userRow = mysqli_fetch_assoc($result)){ $weekNumber = $userRow['week']; }if ($weekNumber == "week 3" ){$weeknumber = $weekNumber; echo '<span class="mode mode_on">done</span>';;}?></td>
                                <td><?php $taskID = $data['usertaskID']; $month = date("F"); $selectUserTask = "SELECT * FROM finishedtask WHERE taskID = '$taskID' AND `month` = '$month'AND `year` = '$year';";$result = mysqli_query($con, $selectUserTask);$weekNumber = '';while($userRow = mysqli_fetch_assoc($result)){ $weekNumber = $userRow['week']; }if ($weekNumber == "week 4" ){$weeknumber = $weekNumber; echo '<span class="mode mode_on"> <a href="" style = "color: white"> done </a></span>';;}?></td>
                                <td><?php $taskID = $data['usertaskID']; $month = date("F"); $selectUserTask = "SELECT * FROM finishedtask WHERE taskID = '$taskID' AND `month` = '$month'AND `year` = '$year';";$result = mysqli_query($con, $selectUserTask);$weekNumber = '';while($userRow = mysqli_fetch_assoc($result)){ $weekNumber = $userRow['week']; }if ($weekNumber == "week 5" ){$weeknumber = $weekNumber; echo '<span class="mode mode_on">done</span>';;}?></td>
