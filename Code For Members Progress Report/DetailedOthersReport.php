<div class="overflow-x" name="OthersReportArea" id="OthersReportArea" style="display: <?php
                     if($othersChecked != ""){
                      echo "block";
                    }
                    else{
                        ...
                        <table class="table table-bordered" class="table datacust-datatable Table ">
                          <thead style="position: sticky; top: -1px;">
                            <tr class="table-dark text-center">
                              <th scope="col" colspan="10" style=" border-bottom: 0px">Detailed Task Report for scheduled task</th>
                            </tr>
                                ...
                                <div class="col-sm-4" style="padding: 0; margin: 0 auto">
                                  <div class="form-group row d-flex justify-content-center">
                                    <form action="<?php $userlevel = $_SESSION['userlevel']; if($userlevel =='Leader'){?>leader.php<?php ; } else if($userlevel =='PIC'){?>userDashBoard.php<?php ; }else{?>admin.php<?php ;} ?>" method="POST">
                                      ...
                                      <input type="date" id="datepickerProgOthers"
                                        value="<?php $startDate = new DateTime($todayOthers);  $startDATE =  $startDate->format('Y-m-d'); echo $startDATE ?>"
                                        name="datepickerProgOthers" style="margin-right: 20px" onchange="filterMonth();">
                                      <label for="colFormLabelLg" class="col-form-label-lg"
                                        style="margin-right: 10px">End</label>

                                      <input type="date" id="datepickerEndProgOthers"
                                        value="<?php $endDate = new DateTime($todayEndOthers); $endDATE =  $endDate->format('Y-m-d'); echo $endDATE ?>"
                                        name="datepickerEndProgOthers" onchange="filterMonth();">
                                      <button type="submit" name="submitdateProgOthers"
                                        class="btn btn-info btn-sm">Submit</button>
                                      <button type="button" class="btn btn-outline-success btn-sm"
                                        onclick="exportDataOthers()"> <i style="font-size: 20px;"
                          ...
                          <tbody class="text-center" id="tableOfOthers">
                            <?php
                              $color1 = "#f9f9f9;";
                              $color2 = "white";
                              $color = "";
                                  if(is_array($fetchDataProgOthers)){      
                                    $sn=1;
                                  foreach($fetchDataProgOthers as $data){
                                    if($sn % 2 == 0){
                                      $color = $color1;
                                    }
                                    else{
                      ...
                      $firstname = $userRow['f_name'];
                              ...
                              <td name="noOfOntime">
                                <?php
                                     $StartDateSelected = new DateTime($todayOthers);
                                       $startDATE =  $StartDateSelected->format('Y-m-d'); 

                                     $DateNowAndToday = new DateTime($todayEndOthers); 
                                     
                                      
                                     $endDATE =  $DateNowAndToday->format('Y-m-d');

                                      $countOthers = "SELECT COUNT(score) as TotalNumberOf1 FROM `finishedtask` WHERE `taskID` = '$userTaskID' AND `score` = '1' AND `sched_Type` = 'others' AND  `realDate` BETWEEN '$startDATE' AND '$endDATE';";
                                      $result = mysqli_query($con, $countOthers);
                                      while($userRow = mysqli_fetch_assoc($result)){
                              ...
                              <td name="noOfLate">
                                <?php
                                     ...
                                     $countOthers = "SELECT COUNT(score) as TotalNumberOf1 FROM `finishedtask` WHERE `taskID` = '$userTaskID' AND `score` = '0.5' AND `sched_Type` = 'others' AND  `realDate` BETWEEN '$startDATE' AND '$endDATE';";
                                     $result = mysqli_query($con, $countOthers);
                                     while($userRow = mysqli_fetch_assoc($result)){
                                     $totalNumberOfScore1 = $userRow['TotalNumberOf1'];
                                     }
                        ...
                        <table style="width:100%; display: none" id="filtertable"
                              ...
                              <td>
                                ...
                                <div class="progress">
                                  <?php
                                        ...
                                        $selectUserTask = "SELECT * FROM usertask WHERE username = '$username' AND Department = '$Department' AND `taskType` != 'others';";
                                        // SELECT week FROM `finishedtask` WHERE `taskID` = '23';
                                        $result = mysqli_query($con, $selectUserTask);
                                        $numOfTask = mysqli_num_rows($result);

                                        $selectUserTaskOthers = "SELECT * FROM usertask WHERE username = '$username' AND Department = '$Department' AND `taskType` = 'others' ;";
                                        // SELECT week FROM `finishedtask` WHERE `taskID` = '23';
                                        $resultothers = mysqli_query($con, $selectUserTaskOthers);
                                        $numOfTaskothers = mysqli_num_rows($resultothers);


                                          //echo("<script>console.log('number of task: " .$numOfTask . "');</script>");
                                        
                                          $weekMonth = weekOfMonth($date_string1);
                                        $selectTaskeme = "SELECT * FROM finishedtask WHERE in_charge = '$username' AND Department = '$Department' AND `month` = '$month1' AND `year` = '$year1' AND `week` = 'week $weekMonth' AND `sched_Type` != 'others';";
                                        ...
                                        // $today = date("F j, Y");
                                        $selectTaskdaiy = "SELECT * FROM finishedtask WHERE in_charge = '$username' AND Department = '$Department'AND `sched_Type` = 'others' AND `Date` = ' $today1' ;";
                                        // SELECT week FROM `finishedtask` WHERE `taskID` = '23';0000

                                        $resultothers2 = mysqli_query($con, $selectTaskdaiy);
                                        $numOfFinishedothers = mysqli_num_rows($resultothers2);

                                        //echo("<script>console.log('number of finished others: " .$numOfFinishedothers . "');</script>");
                                        
                                        // if($numOfFinishedothers == 0){
                                        ...
                                        // else{
                                          ...
                                          $divident1 = $numOfFinished + $numOfFinishedothers;
                                          $divident2 = $numOfTask + $numOfTaskothers;
                          ...
                          <tbody id="tblWeekly" style="display: none">
                            ...
                            <?php
                                ...
                                <tbody id="tblothers" style="display: none">
                                  <?php
                                    $color1 = "#f9f9f9;";
                                    $color2 = "white";
                                    $color = "";
                                      ...
                                      <td>
                                        ...
                                        <div class="progress">
                                          <?php
                                                    ...
                                                    $DepartmentW = $_SESSION['userDept'];
                                                    $selectUserTaskW = "SELECT * FROM usertask WHERE username = '$usernameW' AND Department = '$DepartmentW' AND `taskType` = 'others';";
                                                    ...
                                                    $selectTaskemeW = "SELECT * FROM finishedtask WHERE in_charge = '$usernameW' AND Department = '$DepartmentW' AND `month` = '$month1' AND `year` = '$year1'AND `week` = 'week $weekMonthD' AND `sched_Type` = 'others';";
